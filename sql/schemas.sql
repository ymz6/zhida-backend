...
(
    ...
    app_id         bigint                                not null comment '应用 ID',
    user_id        bigint                                not null comment '用户 ID',
    task_type      varchar(32)                           not null comment '任务类型：CREATE-创建应用，ITERATE-迭代应用，CHAT-对话答疑，DEPLOY-部署应用',
    prompt         text                                  null comment '本次任务的用户提示词或操作说明',
    status         varchar(32) default 'PENDING'         not null comment '任务状态：PENDING-待执行，RUNNING-执行中，SUCCESS-执行成功，FAILED-执行失败',
    current_step   varchar(64)                           null comment '当前执行步骤：INITIALIZING_WORKSPACE-初始化工作区，GENERATING_CODE-生成代码，CHATTING-对话中，BUILDING-构建应用，DEPLOYING-部署应用，FINISHED-已完成',
